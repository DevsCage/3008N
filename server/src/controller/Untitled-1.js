...
exports.feeUpdate = async (req, res) => {
  ...
  Fee.findOne({ stud_reg_num: reqs.stud_reg_num }).exec((err, data) => {
    ...
    if (data) {
      ...
      console.log(find_acadamic_year, "HELLO");
      ...
      if (find_acadamic_year !== undefined) {
        Fee.findOneAndUpdate(
          ...
          if (data) {
            ...
            Student.findOne({ stud_reg_num: stud_reg_num }).exec(
              (error, student) => {
                if (error) {
                  res.status(400).json({
                    message:
                      "Error in finding student, check registration number",
                    error,
                  });
                }
                if (student) {
...
exports.getFeeByStudID = async (req, res) => {
  ...
  Fee.findOne({ stud_reg_num: stud_reg_num })
    // .populate("Student stud_reg_num stud_fname stud_lname")
    .exec((err, data) => {
      if (data) {
  ...
  // Fee.findOne({ stud_reg_num: stud_reg_num }).exec((error, fee) => {
  //   if (fee) {
  //     res.status(200).json({ message: "Fee Found", fee });
  //   } else {
  //     res.status(400).json({
  //       message: "Student and Fee not found, please check registration number",
  //     });
  //   }
  // });
};
